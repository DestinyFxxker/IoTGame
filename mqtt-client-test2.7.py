...
def on_message(client, userdata, msg):
    ...
    if typecode==1:
        ...
        client.publish(deviceName+"_Temp", json.dumps({"devEUI":devEUI,"value":list[1]} ,sort_keys=false), qos=0, )
        client.publish(deviceName+"_XValue", json.dumps({"devEUI":devEUI,"value":list[2]} ,sort_keys=false), qos=0, )
        client.publish(deviceName+"_YValue", json.dumps({"devEUI":devEUI,"value":list[3]} ,sort_keys=false), qos=0,)
        client.publish(deviceName+"_ZValue", json.dumps({"devEUI":devEUI,"value":list[4]} ,sort_keys=false), qos=0, )
        client.publish(deviceName+"_Angle", json.dumps({"devEUI":devEUI,"value":list[4]} ,sort_keys=false), qos=0, )
